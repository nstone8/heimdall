...
class RidgeSpecSemiGutter(RidgeSpec):
    def detect_ridges(self,background:np.ndarray,debug:bool=False):
        ...
        #find the channel area and look for convex corners
        ...
        #pick the line which cuts off the least of the 'ridge' side while resulting in the most objects
        top_edge_sum=sum(channel[0,:])
        bottom_edge_sum=sum(channel[-1,:])

        if top_edge_sum>bottom_edge_sum:
            #need to invert image to count up, will correct for this below
            channel=channel[::-1,:]
